...
- Downloadable CV PDF is generated by `rendercv` from `_data/cv.yml` (see `_pages/cv.md` `cv_pdf` and `_data/socials.yml` `cv_pdf`, both pointing at `assets/rendercv/rendercv_output/Tianyi_Liu_CV.pdf`). The old hand-compiled LaTeX PDF (`assets/pdf/cv_academic_TianyiLiu*.pdf`) was retired 2026-07-23 because it contained career info (an employment gap year) the on-page/rendercv CV intentionally omits — do not resurrect that file or its `cv_pdf` references.
- `requirements.txt` pins `rendercv[full]==2.6` — later versions (2.7+) renamed/removed several `cv:` fields this repo's pre-v1.0 al-folio fork's `_layouts/cv.liquid`/`_includes/cv/*.liquid` still read under the old names (`label`, `image`, `summary`, Education `studyType`/`score`/`courses`, Awards `awarder`, Skills `icon`+`name`+`keywords`). Do not bump this pin without also migrating the CV templates or confirming upstream compatibility — see closed GitHub issue #1 for the full field-rename table.
